...
class TrelloIntegration(BaseIntegration):
    ...
    def create_board(self, board: TrelloBoardCreate) -> TrelloBoard:
        ...
        response = requests.post(
            url=url,
            headers=self.headers,
            params=self._model_to_query_params(board)
        )
        return response.json()
    ...
    def update_board(self, board_update: TrelloBoardUpdate) -> TrelloBoard:
        ...
        response = requests.put(url=url, headers=self.headers, params=self.base_params, json=board_update.dict(exclude_none=True))
        return TrelloBoard(**response.json())
    ...
    def delete_board(self, board_delete: TrelloBoardDelete) -> Any:
        ...
        """
        url = f"{self.base_url}boards/{board_delete.id}/closed"
        response = requests.put(url=url, headers=self.headers, params={**self.base_params, "value": "true"})
        return response.json()

    @task('Create Card')
    def create_card(self, card: TrelloCardCreate) -> TrelloCard:
        """
        Creates a new card.
        """
        url = f"{self.base_url}cards"
        response = requests.post(url=url, headers=self.headers, params=self.base_params, json=card.dict(exclude_none=True))
    ...
    def update_card(self, card_update: TrelloCardUpdate) -> TrelloCard:
        ...
        response = requests.put(url=url, headers=self.headers, params=self.base_params, json=card_update.dict(exclude_none=True))
        return TrelloCard(**response.json())
    ...
    def create_list(self, list_create: TrelloListCreate) -> TrelloList:
        ...
        response = requests.post(url=url, headers=self.headers, params=self.base_params, json=list_create.dict(exclude_none=True))
    ...
    def update_list(self, list_update: TrelloListUpdate) -> TrelloList:
        ...
        response = requests.put(url=url, headers=self.headers, params=self.base_params, json=list_update.dict(exclude_none=True))
        return TrelloList(**response.json())
