...
class TaskPrepAnswer(Base):
    ...
    natural_language_explanation: Mapped[str] = mapped_column(String, nullable=False)
    task_prep_prompt: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("task_prep_prompt.id"), nullable=False)
    task_run: Mapped[UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("task_run.id"), nullable=False)
    body: Mapped[str] = mapped_column(String, nullable=False)
    
    has_python_execution: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)
    ...
    belongs_to_task_run: Mapped["TaskRun"] = relationship("TaskRun", back_populates="task_prep_answers")
