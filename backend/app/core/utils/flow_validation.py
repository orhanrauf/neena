    ...
    for task_op in flow.task_operations:
        if task_op.name in task_op_names:
            validation_messages.append(
                FlowValidationFailureMessage(
                    f"Task operation name '{task_op.name}' is used more than once. Each Task operation in a Flow must have a unique name."
                )
            )
        else:
        ...
        if task_definition is None:
            validation_messages.append(
                TaskValidationFailureMessage(
                    f"Task definition for task {task_op.name} does not exist in the database.", task_name=task_op.name
                )
            )
        else:
...
def validate_task(
    task_op: schemas.TaskOperationBase, task_definition: TaskDefinitionBase, flow_request: FlowRequestBase
) -> list[ValidationMessageBase]:
    validation_messages = []
    ...
    if not task_op.name or not re.match("^[a-zA-Z0-9_-]*$", task_op.name):
        validation_messages.append(
            TaskValidationFailureMessage(
                f"Attribute 'name' for task cannot be empty or and can only contain letters, numbers, underscores ('_'), and hyphens ('-').",
                task_name=task_op.name,
            )
        )
...
def check_task_attributes_empty(
    task_op: schemas.TaskOperationBase,
) -> Generator[FlowValidationFailureMessage, None, None]:
    ...
    for attr_name, attr_type in attributes.items():
        ...
        if attr_name not in task_op.__dict__:
            yield TaskValidationFailureMessage(
                f"Attribute '{attr_name}' cannot be empty for {task_op.name}", task_name=task_op.name
            )
    return


def check_task_arguments(
    task_op: schemas.TaskOperationBase, task_definition: schemas.TaskDefinitionBase, flow_request: FlowRequestBase
) -> list[ValidationMessageBase]:
    validation_messages = []
    ...
    for arg in task_op.arguments:
        try:
            if arg.data_type != params[arg.name]:
                validation_messages.append(
                    TaskValidationFailureMessage(
                        f"Data type of argument {arg.name} does not match the corresponding parameter in task {task_op.name}.",
                        task_name=task_op.name,
                    )
                )
            if arg.source not in valid_sources:
                validation_messages.append(
                    TaskValidationFailureMessage(
                        f"Invalid source for argument {arg.name} in task {task_op.name}. Source should be one of @tasks(), @context(), @metadata().",
                        task_name=task_op.name,
                    )
                )
            if arg.source == "@metadata()":
                if not any(arg.value in d.values() for d in flow_request.request_metadata):
                    validation_messages.append(
                        TaskValidationFailureMessage(
                            f"Value {arg.value} for argument {arg.name} in task {task_op.name} does not exist in the flow request metadata.",
                            task_name=task_op.name,
                        )
                    )
            ...
            validation_messages.append(
                TaskValidationFailureMessage(
                    f"Argument {arg.name} for task {task_op.name} does not exist in task definition parameters.",
                    task_name=task_op.name,
                )
            )
...
def check_edge_validity(
    task_op: schemas.TaskOperationBase, flow: schemas.FlowBase, db: Session
) -> list[ValidationMessageBase]:
    validation_messages = []
    ...
    for arg in task_op.arguments:
        if arg.source == "@tasks()":
            ...
            if "." not in arg.value or arg.value.split(".")[-1] != "output":
                validation_messages.append(
                    TaskValidationFailureMessage(
                        f"Invalid value format for argument {arg.name} in task {task_op.name}. When the source is '@tasks()', value should be in the format of 'task_name.output'.",
                        task_name=task_op.name,
                    )
                )
                continue
            ...
            if not source_task_op:
                validation_messages.append(
                    TaskValidationFailureMessage(
                        f"The source task operation {task_output_source} does not exist in the flow for task {task_op.name}.",
                        task_name=task_op.name,
                    )
                )
                continue
            ...
            if source_task_definition is None:
                validation_messages.append(
                    TaskValidationFailureMessage(
                        f"Task definition for task {source_task_op.name} does not exist in the database.",
                        task_name=source_task_op.name,
                    )
                )
                continue
            ...
            if arg.data_type != source_task_definition.output_type:
                validation_messages.append(
                    TaskValidationFailureMessage(
                        f"The data type of argument {arg.name} does not match the output type of the source task operation {task_output_source} for task {task_op.name}.",
                        task_name=task_op.name,
                    )
                )
