...
class FlowGenerator:
    ...
    def generate_flow_from_request(self, request: str) -> FlowBase:
        ...
        """
        request_task_definitions = (
            self._get_task_definitions_from_database(request)
            if self.fake_rag
            else self._retrieve_task_definitions_from_database(request)
        )
    ...
    def _retrieve_task_definitions_from_database(self, request: str) -> list[TaskDefinition]:
        ...
        messages = [
            {
                "role": "system",
                "content": """
                    You are part of an application that enables its users to construct and automate complex flows in and between Trello and Slack. 
                    ...
                    List of permissible Trello task names:
                        Create Board
                        Create Card
                        Create List
                        Delete Board
                    ...
                    List of permissible Slack task names:
                        Send Message
                        Update Message
                        Delete Message
            ...
            {
                "role": "user",
                "content": "Find and delete the project planning card about migrating the sql database, and send a message notifying everbody in the #db-migration channel.",
            },
            {"role": "assistant", "content": '["Get Boards", "Get List", "Get Card", "Delete Card", "Send Message"]'},
    ...
    def _construct_flow(self, task_operations: list[TaskOperationBase], dependencies: DependencyList) -> FlowBase:
        flow = FlowBase(
            name=f"[Flow Name Placeholder]", task_operations=task_operations, dependencies=dependencies.dependencies
        )
        return flow
