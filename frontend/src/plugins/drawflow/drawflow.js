export default class Drawflow {
  constructor(container, render = null, parent = null) {
    ...
    this.precanvas = null;
    this.nodeId = 1; // first node Id
    this.ele_selected = null; // Current element selected
    this.node_selected = null; // Current node selected
    this.drag = false; 
    this.reroute = false;
    ...
    this.connection = false; // To track if a connection is being dragged
    this.connection_ele = null; // DOM element of the connection being dragged/selected
    this.connection_selected = null; // check if connection is selected
    this.canvas_x = 0;
    this.canvas_y = 0;
    this.pos_x = 0;
    this.pos_x_start = 0;
    ...
    this.mouse_y = 0;
    this.line_path = 5;
    this.first_click = null; // To track what was clicked first after clicking a second time or ending a drag
    this.force_first_input = false;
    this.draggable_inputs = true;
    this.useuuid = false;
    this.parent = parent; // app context for Vue
  ...
  click(e) {
    ...
    if(this.editor_mode === 'fixed') {
      ...
      if(e.button === 0) {
        // regardless of when and where, remove any context menu elements (X)
        this.contextmenuDel();
      }
      if(e.target.closest(".drawflow_content_node") != null) {
        this.ele_selected = e.target.closest(".drawflow_content_node").parentElement; // Gets the DrawFlow node container, as opposed to the underlying Vue object
      }
    }
    switch (this.ele_selected.classList[0]) {
      ...
      case 'output': // 
        ...
        break;
      case 'parent-drawflow': // in case the clicked-on element is the parent-drawflow, deselect all nodes or connections
        if(this.node_selected != null) {
          this.node_selected.classList.remove("selected");
          this.node_selected = null;
          this.dispatch('nodeUnselected', true);
  ...
  position(e) {
    // Called on each mousemove or touchmove event
    if (e.type === "touchmove") {
  ...
  dragEnd(e) {
    ...
    Gets triggered when mouse goes up or touch ends
    Used to stop dragging node or connection
  ...
  // Might delete this functionality later on to build a custom context menu
  contextmenu(e) {
    // Triggered when clicking the right mouse button
  ...
  contextmenuDel() {
    // Removes any context menu elements
    if(this.precanvas.getElementsByClassName("drawflow-delete").length) {
      this.precanvas.getElementsByClassName("drawflow-delete")[0].remove()
    };
  }
  ...
  // Adds connection based on the node ids and and their input/output
  // If it already exists, does nothing but dispatch connectionCreated event
  ...
  dblclick(e) {
    try {
      const listclassConnection = this.connection_selected.parentElement.classList;
      if(listclassConnection.length > 1){
        console.log('botototo')
        this.dispatch('connectionDoubleClicked', { output_id: listclassConnection[2].slice(14), input_id: listclassConnection[1].slice(13), output_class: listclassConnection[3], input_class: listclassConnection[4] });
      }
    }
    catch(err) {
      if(err && !err.message.includes('Cannot read properties')) {
        throw err;
      }
    }
  }
  ...
  addNode (name, num_in, num_out, ele_pos_x, ele_pos_y, classoverride, data, html, typenode = false) {
    ...
    if (!this.useuuid) {
      this.nodeId++; // Increment the node number, the nodee Ids are ever-growing integers - if a node gets removed, the next one will still have a higher number
    }
    return newNodeId;
  }
  ...
  getModuleFromNodeId(id) {
    // Loops over each module to check which one contains the node with the given id
    var nameModule;
    const editor = this.drawflow.drawflow
    Object.keys(editor).map(function(moduleName, index) {
  ...
  on (event, callback) {
   ...
   // dispatches an event by name to the registered listeners
