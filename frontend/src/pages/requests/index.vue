...
function navigateToRequest(event, item) {
    router.push(`/requests/${item.item.id}`);
}

// Headers
const headers: any = [
    { title: 'Request', key: 'request_instructions', width: '50%' },
    { title: 'HAS FLOW', key: 'flow', align: 'start', width: '15%' },
    {
        title: 'CREATED DATE',
        key: 'created_date',
        align: 'start',
...
let allFlowRequests = store.state.flowRequests;
let flowRequests = ref([]);

function filterRequests(): any {
    const searchTerm = search.value.toLowerCase();
    flowRequests.value = searchTerm
        ? allFlowRequests.filter((request: any) =>
              request.request_instructions.toLowerCase().includes(searchTerm)
          )
        : [...allFlowRequests];
}
...
async function deleteRequest(id: string) {
    let res = await store.dispatch('deleteFlowRequests', id);
    if (res === 200) {
        flowRequests.value = flowRequests.value.filter(
            (req: any) => req.id !== id
        );
    }
}
...
<template>
    ...
    <VCard class="requests">
        ...
        <VDataTable
            ...
            @click:row="navigateToRequest"
        >
            <!-- details -->
            <template #item.request_instructions="{ item }">
                <span
                    v-html="item.request_instructions"
                    class="color-primary"
                ></span>
            </template>
            ...
            <template #item.flow="{ item }">
                <span
                    class="has-flow"
                    :class="item.flow ? 'has-flow-yes' : 'has-flow-no'"
                >
                    {{ item.flow ? 'Yes' : 'No' }}
                </span>
            </template>
            ...
            <template #item.actions="{ item }">
                <button class="mx-1" @click="deleteRequest(item.id)">
                    <VIcon
                        icon="tabler-trash"
                        class="color-primary"
                        size="26"
