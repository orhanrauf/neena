...
import { TaskDefinition, TaskDefinitionParameter } from "@/types"
...
const generateRandomTaskDefinitions = (count: number): TaskDefinition[] => {
  const taskDefinitions: TaskDefinition[] = [];
  for (let i = 0; i < count; i++) {
    taskDefinitions.push({
      id: faker.datatype.uuid(),
      task_name: faker.lorem.sentence(),
      parameters: generateRandomParameters(faker.datatype.number({ min: 1, max: 5 })),
      description: faker.lorem.sentence(),
      output_type: getRandomElement(["str", "int", "list", "object", "bool"]),
      python_code: faker.hacker.phrase(),
      created_date: faker.date.past().toISOString(),
      modified_date: faker.date.recent().toISOString(),
      created_by_email: faker.internet.email(),
      modified_by_email: faker.internet.email(),
      deleted_at: faker.datatype.boolean() ? faker.date.recent().toISOString() : null
    });
  }
  return taskDefinitions;
};

const generateRandomParameters = (count: number): TaskDefinitionParameter[] => {
  const parameters: TaskDefinitionParameter[] = [];
  for (let i = 0; i < count; i++) {
    parameters.push({
      name: faker.hacker.noun(),
      data_type: getRandomElement(["str", "int", "list", "object", "bool"]),
      position: i + 1
    });
  }
  return parameters;
...
// Use this method to generate mock data
const items = ref(generateRandomTaskDefinitions(100)); // Adjust the number as needed
/////////////////////////////
  ...
  <VCard>
    ...
    >
      ...
      <template #item.name="{ item }">
        <span class="text-prim" v-html="item.name"></span>
      </template>
    

      <!-- Parameters -->
      <template #item.parameters="{ item }">
        <span v-html="item.parameters"></span>
      </template>
